...
drag_house_frame <- tibble(wiki_root_url,drag_house_urls) %>% 
  ...
  filter(!(url_end %in% c("Category:House_of_Stupi","Template (Family Tree)"))
         ) %>% 
  mutate( html_results = map(  drag_house_frame$family_url
                             , ~ {.x %>%
                                     read_html()
                                  }
                             )
         , all_names=map(   drag_house_frame$html_results
                        , ~{.x %>% 
                              html_nodes(".category-page__member") %>% 
                              html_text() %>% 
                              str_replace_all( "[\t\n]" , "")
...
```{r using map to get all of the names in the house and then unnesting them into one column, include=FALSE}
